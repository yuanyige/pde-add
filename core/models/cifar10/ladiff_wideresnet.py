...
class Diffusion(nn.Module):
    def __init__(self, planes):
        ...
        self.main = nn.Sequential(
            ...
            # nn.Conv2d(2*planes, 2*planes, kernel_size=3, stride=1, padding=1, bias=True),
            # nn.BatchNorm2d(2*planes),
            # nn.ReLU(inplace=True),
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, stride, dropRate=0.0):
        ...
        self.diff = Diffusion(out_planes)
    ...
    def forward(self, x):
        ...
        # return out + 0.1 * torch.randn_like(out) * torch.std(out)
...
class WideResNet(nn.Module):
    ...
    def net(self, x, use_diffusion=True):
        ...
        out = self.conv1(x)
        out = self.block1((out, use_diffusion, 0))
