...
class PGDAttack(Attack, LabelMixin):
    ...
    def __init__(
            ...
            ord=np.inf, targeted=False, rand_init_type='uniform',save_path=None):
    ...
    def vis(self, x_ori, x_aug):
        ...
        if self.ord == np.inf:
            ...
            o = '2'
        save_image(x.cpu(), os.path.join(self.save_path,'adv-l{}.png'.format(o)), nrow=8,
            padding=0, value_range=(0, 1), pad_value=0)

    def perturb(self, x, y=None, visualize=False):
        """
...
class LinfPGDAttack(PGDAttack):
    ...
    def __init__(
            self, predict, loss_fn=None, eps=0.3, nb_iter=40, eps_iter=0.01, rand_init=True, clip_min=0., clip_max=1.,
            targeted=False, rand_init_type='uniform',save_path=None):
        ord = np.inf
        super(LinfPGDAttack, self).__init__(
            ...
            clip_min=clip_min, clip_max=clip_max, targeted=targeted, ord=ord, rand_init_type=rand_init_type, save_path=save_path)
